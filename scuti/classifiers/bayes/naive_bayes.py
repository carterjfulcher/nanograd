...
class BayesClassifier(): 
  ...
  def _feature_probability(self, _class, x): #compute the probabilty of feature | class 
    mean = self._normal_dist[_class]['mean'] 
    std = self._normal_dist[_class]['std'] ** 2
    return 1 / math.sqrt(2 * math.pi * self._std ** 2) * math.exp(-(x - mean) ** 2 / (2 * std))
  ...
  def fit(self, features, labels): 
    ...
    for _class, _ in self._normalizing_constants.items():
      ...
      self._normal_dist[_class] = {}
      self._normal_dist[_class]['mean'], self._normal_dist[_class]['std'] = Distribution.normal_distribution(
            np.array(class_filtered)
          ) 
      print(self._normal_dist)
